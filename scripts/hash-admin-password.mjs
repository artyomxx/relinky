#!/usr/bin/env node
/**
 * Print a sha512-crypt ($6$) line for RELINKY_ADMIN_PASSWORD_HASH.
 * Same format as: openssl passwd -6
 *
 * Usage: node scripts/hash-admin-password.mjs <plaintext-password> [--b64]
 * Rounds: RELINKY_ADMIN_PASSWORD_SHA512_ROUNDS (default 5000)
 *
 * --b64: also print a base64 line for RELINKY_ADMIN_PASSWORD_HASH_B64 (Coolify-safe, no $ characters).
 */
...
if (printB64) {
	console.error('')
	console.error('RELINKY_ADMIN_PASSWORD_HASH_B64 (paste into Coolify as Literal; no $ in this value):')
